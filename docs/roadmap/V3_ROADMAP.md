...
- `UN-TRIAD-MIX` / `UN-GWM-V2`: pure feature extraction and instruction-channel descriptor branch for treating each ordered point triad as deterministic point, edge, triangle, optional context, and descriptor material. Sprint 32 emits descriptors only, does not apply `UN-ROTATE`, `UN-SWAP`, or permutation transforms, and does not change existing `UN-GWM` streams.
