...
- `UN-TRIAD-MIX` / `UN-GWM-V2`: pure feature extraction, instruction-channel descriptor, opt-in stream descriptor, opt-in stream adapter, and opt-in transform proof branch for treating each ordered point triad as deterministic point, edge, triangle, optional context, descriptor, and application-proof material. Sprint 35 transform proof output is deterministic, not random, does not create a production cipher mode, and does not change existing `UN-GWM` streams.
...
- Triad default integration follow-up: any default transform path for triad output must be a separate explicit versioned mode with tests; Sprint 35 remains an isolated opt-in proof only.
