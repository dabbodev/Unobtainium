...
- `UN-TRIAD-MIX` / `UN-GWM-V2`: pure feature extraction, instruction-channel descriptor, and opt-in stream descriptor branch for treating each ordered point triad as deterministic point, edge, triangle, optional context, and descriptor material. Sprint 33 emits descriptors only, does not apply `UN-ROTATE`, `UN-SWAP`, or permutation transforms, and does not change existing `UN-GWM` streams.
...
- Triad transform adapter follow-up: define opt-in transform translation tests for triad-mix output without changing legacy streams by default.
