...
- `UN-TRIAD-MIX` / `UN-GWM-V2`: pure feature extraction, instruction-channel descriptor, opt-in stream descriptor, opt-in stream adapter, isolated opt-in transform proof, Sprint 38 future mode specification, Sprint 39 pure descriptor utilities, Sprint 40 source point commitment plus opt-in triad stream generation utilities, Sprint 41 supplied adapter-plan binding utilities, Sprint 42 supplied transform-proof binding utilities, Sprint 43 consolidation/readiness checks, Sprint 44 true opt-in mode wrapper, and Sprint 45 commit-readiness consolidation for treating each ordered point triad as deterministic point, edge, triangle, optional context, descriptor, binding, and optional proof material. Sprint 45 does not implement default `UN-GWM-V2`, does not create a production cipher mode, does not generate adapter plans or transform proofs, does not apply transform proofs or transforms, does not add a default migration, and does not change existing `UN-GWM` streams.
