...
`UN-GWM-V2`: A future explicit opt-in geometric walk mask mode using the existing `UN-TRIAD-MIX` pipeline. Sprint 38 defines the conceptual descriptor shape, pipeline stages, compatibility rules, test-vector strategy, API sketches, and non-goals in `docs/specs/UN-GWM-V2_IDEAS.md`. Sprint 39 adds pure committed descriptor utilities only. Existing `UN-GWM` streams must not change unless a future explicit version or format is introduced.
