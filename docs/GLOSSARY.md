...
UN-GWM-V2: A future explicit opt-in geometric walk mask mode based on the existing `UN-TRIAD-MIX` pipeline. Sprint 38 defines it as a separate mode with its own format/version/mode marker, source point commitment, walk options, triad stream commitment, adapter plan commitment, optional transform proof commitment, optional context/metadata, and descriptor commitment. Sprint 39 adds pure committed descriptor utilities. Sprint 40 adds source point commitments and opt-in triad stream generation for descriptors only; it does not generate adapter plans, apply transform proofs, apply transforms, or add runtime integration. It must not change existing `UN-GWM` instruction streams unless a future explicit version or format is introduced.
