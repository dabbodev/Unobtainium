...
Status: Sprint 31 first-pass implementation. `UN-TRIAD-MIX` now has pure feature extraction utilities under `packages/core/src/triad-mix.js`. This sprint does not emit transform instructions and does not change legacy runtime behavior, existing `UN-GWM` behavior, instruction streams, `UN-ROTATE`, `UN-SWAP`, stack/cascade/cert/cutout integration, CLI behavior, browser behavior, or file behavior.

`UN-TRIAD-MIX` is experimental deterministic feature extraction and future feature mixing. It is not production cryptography, not cryptographic randomness, not authenticated encryption, not asymmetric cryptography, not secure redaction, not compression, not steganography, not tamper-proofing, and not a production-safe cipher claim.
...
Sprint 31 implements only the deterministic bundle of point, edge, triangle, order, and optional walk-context features. The same selected triad may later produce value material for `UN-ROTATE`, position material for `UN-SWAP` or future permutation plans, rule-selection material for angle-dependent mixing, and explanation material for diagnostics and test vectors, but that emission is future scope.

This is deterministic feature extraction, not randomness. Identical inputs, options, and walk context reproduce identical feature payloads and commitments. Contextual dependency should not be marketed as cryptographic uncertainty. More feature mixing does not automatically mean more security. Any future transform that consumes emitted instructions remains responsible for reversibility and bounds.
...
Feature extraction keeps feature groups visible so test vectors can explain which part of the triad influenced the payload. Sprint 31 exposes point, edge, triangle, and optional walk-context material only; it does not produce rotate, swap, permutation, rule, or debug instruction channels.
...
Single-point features should not be used in isolation for any future emitted instruction unless another feature from B, C, an edge, the whole triangle, or walk state also participates. Sprint 31 does not emit instructions.
...
Matrix, N-dimensional, or mutation-derived context is future-only. Sprint 31 does not define matrix integration, N-dimensional angle implementation, matrix mutation integration, or hidden key-state evolution.
...
`UN-TRIAD-MIX` should be modeled as a future multi-channel instruction emitter. Sprint 31 does not emit channels for each ordered triad.

### Rotate / Value Channel

The rotate/value channel emits bounded value material for `UN-ROTATE` instructions. It may produce a raw shift, adjusted shift, window size, min-shift diagnostic, and source feature references. The channel must stay within the active ring/window bounds.

### Swap / Position Channel

The swap/position channel emits bounded position material for `UN-SWAP`, pair-swap plans, or future permutation plans. It may produce position candidates, pair selectors, stride material, or permutation seeds. Any emitted position must be normalized against the payload length or declared permutation domain before a transform consumes it.

### Rule / Mix Channel

The rule/mix channel selects deterministic mixing patterns. It may use angle bucket, orientation, edge balance, centroid signature, or walk index to pick a named pattern. The selected pattern should be recorded by ID so test vectors can assert the selection without re-deriving prose behavior.

### Explain / Debug Channel

The explain/debug channel emits stable diagnostic material for deterministic tests and inspection. It should record selected indexes, copied points or point commitments, feature summaries, bucket IDs, pattern IDs, channel outputs, bounds, and before/after walk state.
...
Potential future channels include gate hints, cutout hints, certificate hints, cascade hints, or stack policy hints. These are future-only and are not integrated by Sprint 31. Hints must not be described as authorization, secure redaction, identity proof, ownership proof, or production authentication.

## Mixing Patterns

The first future instruction-emission implementation should use a small named set of deterministic, test-vector-friendly pattern concepts. These are concepts only; Sprint 31 does not implement them.
...
A future `UN-TRIAD-MIX` implementation should be opt-in. Existing v3 instruction streams must not change unless a new version, format, or explicit generator option is introduced.
...
Existing `UN-GWM` instruction stream generation remains its own compatibility path. `UN-GWM-V2` should not reinterpret existing streams, silently change stack layer meaning, or alter existing transform behavior.

## Test-Vector Strategy

Tests should prove deterministic feature extraction first, then emitted channel bounds in a later sprint before any transform integration is trusted.
...
Sprint 31 test vectors include compact triads, expected point summaries, edge summaries, whole-triangle summaries, context commitment changes, and degenerate cases. Future instruction-emission tests may add selected pattern IDs, emitted channel values, and explain/debug output. Tests should avoid brittle prose assertions.
...
- Sprint 32: add multi-channel instruction emission for rotate/value, swap/position, rule/mix, and explain/debug channels while preserving existing instruction stream compatibility.
