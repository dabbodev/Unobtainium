...
See the [v3 roadmap](docs/roadmap/V3_ROADMAP.md) for current module status and future design branches. Raw v3 modes are not production encryption. `UN-TRIAD-MIX` is currently an experimental opt-in branch for triad feature extraction, instruction-channel descriptors, stream descriptors, adapter descriptors, and isolated transform proof helpers. `UN-GWM-V2` currently has descriptor, source point commitment, opt-in stream generation, supplied adapter binding, supplied proof binding, and explicit opt-in mode wrapper utilities. Sprint 45 is consolidation and commit-readiness only. It does not add a default `UN-GWM-V2` mode, default migration, automatic adapter plan creation, automatic transform proof creation, transform proof application, transform application, runtime integration, CLI/file wrappers, or browser playground behavior. Existing `UN-GWM` remains unchanged. Hybrid production crypto envelopes remain future scope and should preserve the project as an experimental geometry-key lab rather than flattening it into "just use AES."
