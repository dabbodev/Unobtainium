...
The current v3 line includes `UN-MATRIX`, `UN-MATRIX-MUTATE`, `UN-MATRIX-MUTATE-SIGNED`, `UN-MATRIX-COMBINE`, `UN-MATRIX-COMBINE-SIGNED`, `UN-CERT`, `UN-CUTOUT` / `UN-STENCIL`, `UN-CERT` cutout binding helpers, and opt-in `UN-TRIAD-MIX` feature/channel/stream/adapter/proof helpers. These are experimental object, recipe, validation, feature, descriptor, and data-transformation primitives. Commitments are deterministic integrity/check artifacts, and signed envelopes are signer-intent wrappers over committed payloads. They are not production cryptography, authenticated encryption, asymmetric cryptography, secure redaction, safe key evolution, compression, steganography, ownership proof, identity proof, or authorization systems.
...
See the [v3 roadmap](docs/roadmap/V3_ROADMAP.md) for current module status and future design branches. Raw v3 modes are not production encryption. `UN-TRIAD-MIX` is currently an experimental opt-in branch for triad feature extraction, instruction-channel descriptors, stream descriptors, adapter descriptors, and isolated transform proof helpers; default `UN-GWM-V2` integration remains future scope. CLI/file wrappers, browser playground work, and hybrid production crypto envelopes remain future scope. A future hybrid envelope should preserve the project as an experimental geometry-key lab rather than flattening it into "just use AES."
