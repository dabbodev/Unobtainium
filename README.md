...
See the [v3 roadmap](docs/roadmap/V3_ROADMAP.md) for current module status and future design branches. Raw v3 modes are not production encryption. `UN-TRIAD-MIX` is currently an experimental opt-in branch for triad feature extraction, instruction-channel descriptors, stream descriptors, adapter descriptors, and isolated transform proof helpers. Sprint 40 adds source point commitments and opt-in triad stream generation to the [`UN-GWM-V2` descriptor utilities](docs/specs/UN-GWM-V2_IDEAS.md) for a future explicit opt-in path only; default behavior, adapter plan creation, transform proof application, transform application, and runtime integration remain future scope. Existing `UN-GWM` remains unchanged. CLI/file wrappers, browser playground work, and hybrid production crypto envelopes remain future scope. A future hybrid envelope should preserve the project as an experimental geometry-key lab rather than flattening it into "just use AES."
