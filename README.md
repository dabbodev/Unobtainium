...
> [!WARNING]
> Unobtainium is an experimental masking/data-transformation project and computational thought experiment built around its original “3-dimensional hex-based Enigma-like” idea. Raw legacy and v3 modes are not production cryptography. Do not use them to protect secrets, credentials, regulated data, or adversarial communications. Legacy encryption wording and demos are retained as project archaeology and behavior documentation, not as security claims.
...
The current v3 line includes `UN-MATRIX`, `UN-MATRIX-MUTATE`, `UN-MATRIX-MUTATE-SIGNED`, `UN-MATRIX-COMBINE`, `UN-MATRIX-COMBINE-SIGNED`, `UN-CERT`, `UN-CUTOUT` / `UN-STENCIL`, `UN-CERT` cutout binding helpers, opt-in `UN-TRIAD-MIX` feature/channel/stream/adapter/proof helpers, and `UN-GWM-V2` descriptor, source point commitment, opt-in triad stream generation, supplied adapter-plan binding, supplied transform-proof binding, and true opt-in mode wrapper utilities. These are experimental object, recipe, validation, feature, descriptor, binding, wrapper, and data-transformation primitives. Commitments are deterministic integrity/check artifacts, and signed envelopes are signer-intent wrappers over committed payloads. They are not production cryptography, authenticated encryption, asymmetric cryptography, secure redaction, safe key evolution, compression, steganography, ownership proof, identity proof, or authorization systems.
...
See the [v3 roadmap](docs/roadmap/V3_ROADMAP.md) for current module status and future design branches. Raw v3 modes are not production encryption. `UN-TRIAD-MIX` is currently an experimental opt-in branch for triad feature extraction, instruction-channel descriptors, stream descriptors, adapter descriptors, and isolated transform proof helpers. `UN-GWM-V2` currently has descriptor, source point commitment, opt-in stream generation, supplied adapter binding, supplied proof binding, and explicit opt-in mode wrapper utilities. Sprint 46 adds a docs-only [future visual/demo bridge plan](docs/specs/UN-GWM-V2_VISUAL_DEMO_IDEAS.md), Sprint 47 adds a small static [visual demo fixture](docs/examples/gwm-v2-visual-demo-fixture.json) for future JSON-driven panels, Sprint 48 validates that fixture shape only, Sprint 49 defines a static demo scaffold specification only, Sprint 50 adds a minimal static [fixture viewer](demo/gwm-v2-visual/) that renders the checked-in fixture into local HTML panels, Sprint 51 refines its point/walk/triad/feature panels, Sprint 52 refines its stream/adapter/proof/descriptor panels, and Sprint 53 consolidates the static visual-demo arc for commit readiness without adding demo behavior. These explain the pipeline as local explanatory playground material; they do not implement live core execution, CLI/file wrappers, transform behavior, runtime integration, WebGL/canvas behavior, file upload, or changes to existing `UN-GWM`. The fixture and scaffold are explanatory and non-production; they are not security test vectors. Hybrid production crypto envelopes remain future scope and should preserve the project as an experimental geometry-key lab rather than flattening it into "just use AES."
